...
**Layer console (`/layer`) — OpenClaw demo:** bundled tunnel URLs and tokens live in **`client/src/lib/layerDemoDefaults.js`** (update when ngrok rotates). Click **Autofill demo** with no setup, or set optional `VITE_LAYER_*` in `client/.env` to override. Restart Vite after `.env` changes. **Register & delegate on-chain** calls **`POST /api/layer/apply`** (`server/lib/layerApplyWorkflow.mjs`): three ERC-8004 registrations (orchestrator / research / payment from `agentSlot`), root Biscuit mint, then two **`delegate()`** hops (O→R, R→P) with per-node `allowedTools` / spend caps. Requires **`PRIVATE_KEY`** and deployed addresses; response **agent bindings** (IDs + OpenClaw base per slot) are merged into **`server/data/layer-snapshot.json`**. **Run OpenClaw chain** runs Orchestrator → Research → Payment over HTTP. **Run DCT live demo** navigates to **`/live-demo`** and auto-starts the full E2E. Chat and `/health` are **proxied** via `POST /api/layer/openclaw-chat` and `GET /api/layer/openclaw-health`. Run **`cd server && npm start`** alongside Vite. For non-ngrok OpenClaw hosts, set **`LAYER_OPENCLAW_ALLOW_ALL=1`** on the server (see `server/routes/layer-workflow.js`).

**Landing hero — delegation panel:** fetches **`GET /api/delegation/tree`** and **`GET /api/agents`** for live counts; the SVG remains an illustrative animation.

**Layer — BTC + TLS + OpenClaw:** button runs **`POST /api/tlsn/prove`** against **`VITE_BTC_TLS_DEMO_URL`** (default CoinGecko BTC/USD), then sends the verified response preview to the **Research** OpenClaw agent. Requires **`TLSN_PROVER_URL`** and a running notary/prover stack (`npm run tlsn-prover`, Docker notary, etc.); see **TLSNotary** below.
